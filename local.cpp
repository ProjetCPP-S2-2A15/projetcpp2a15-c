...
bool Local::ajouter()
{
    ...
    query.prepare("INSERT INTO FETHI.local_commerciale ( SURFACE, TYPE, DISPONIBILITE, PRIX, ETAGE) "
...
bool Local::supprimer(long long id) {
    ...
    query.prepare("DELETE FROM FETHI.local_commerciale WHERE ID_LOCAL = :ID_LOCAL");
...
bool Local::modifier(long long id) {
    ...
    query.prepare("UPDATE FETHI.local_commerciale SET "
                  "SURFACE = :SURFACE, "
                  "TYPE = :TYPE, "
...
QStandardItemModel*  Local::afficher()
{
    ...
    QSqlQuery query("SELECT ID_LOCAL, SURFACE, TYPE, DISPONIBILITE, PRIX, ETAGE FROM FETHI.local_commerciale");
...
QSqlQueryModel* Local::trier()
{
    ...
    QSqlQuery query;
    query.prepare("SELECT * FROM FETHI.local_commerciale ORDER BY PRIX ASC");  // Trier par prix
...
QSqlQueryModel* Local::rechercherParDisponibilite(const QString& disponibilite)
{
    ...
    query.prepare("SELECT ID_LOCAL, SURFACE, TYPE, DISPONIBILITE, PRIX, ETAGE FROM FETHI.local_commerciale WHERE DISPONIBILITE = :DISPONIBILITE");
    query.bindValue(":DISPONIBILITE", disponibilite);
