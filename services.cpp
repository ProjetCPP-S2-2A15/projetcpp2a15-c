...
QSqlQueryModel* Services::recherchernom(QString nom)
{
    QSqlQuery query;
    query.prepare("SELECT * FROM SERVICES WHERE NOM = :nom");
    query.bindValue(":nom", nom);
    query.exec();

    QSqlQueryModel *model = new QSqlQueryModel;
    model->setQuery(query);  // Consider checking the validity of query execution
...
{
    QSqlQueryModel* model = new QSqlQueryModel();
    model->setQuery("SELECT * FROM SERVICES ORDER BY TYPE ASC");
